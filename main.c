...
int getTopDisk(int k, int* gameState, int* n)
{
	int i = 0;
	while (i < DISKS_NO) {
		int val = *(gameState + i * POLES_NO + k);
		if (val != 0) {
			*n = i;
			return val;
		}
...
void moveDisk(int first, int second, int* gameState)
{
	...
	int sTopRowIndex = 0;
	int targetRow = 0;
	int fTop = getTopDisk(first, gameState, &fTopRowIndex);
	int sTop = getTopDisk(second, gameState, &sTopRowIndex);
	...
	*(gameState + targetRow * POLES_NO + second) = fTop;
	*(gameState + fTopRowIndex * POLES_NO + first) = 0;
}
...
void drawDisks(int* gameState, AnimatedDisk* animatedDisk)
{

	for (int i = 0; i < DISKS_NO; i++) {
		for (int j = 0; j < POLES_NO; j++) {
			if (*(gameState + i * POLES_NO + j) == 0)
				continue;
			if (animatedDisk->active && i == animatedDisk->fromRow &&
				j == animatedDisk->fromPole) {
				continue;
			}

			drawDiskAtPosition(*(gameState + i * POLES_NO + j),
							   (float)poleCenterX(j),
...
void drawAnimatedDisk(AnimatedDisk* animatedDisk)
{
	...
	drawDiskAtPosition(animatedDisk->diskSize, animatedDisk->centerX,
					   animatedDisk->bottomY);
}

void startDiskAnimation(int first, int second, int* gameState,
						AnimatedDisk* animatedDisk)
{
	int fromRow = 0;
	int targetTopRow = 0;
	int targetRow = 0;
	int movingDisk = getTopDisk(first, gameState, &fromRow);
	int targetTopDisk = getTopDisk(second, gameState, &targetTopRow);

	if ((first == second || movingDisk == 0) ||
		(targetTopDisk != 0 && movingDisk > targetTopDisk)) {
		return;
	}
...
void animatedDiskPhase1(AnimatedDisk* animatedDisk, float distance,
						float targetX)
{
...
void updateAnimation(AnimatedDisk* animatedDisk, int* gameState)
{
	...
	if (animatedDisk->bottomY >= animatedDisk->targetBottomY) {
		...
		moveDisk(animatedDisk->fromPole, animatedDisk->toPole, gameState);
...
void populateGameState(int* gameState)
{
	...
	for (int i = 0; i < DISKS_NO; i++) {
		for (int j = 0; j < POLES_NO; j++) {
			if (j == 0) {
				*(gameState + i * POLES_NO + j) = i + 1;
			}
			else {
				*(gameState + i * POLES_NO + j) = 0;
			}

			//printf("%d", *(gameState + i * POLES_NO + j));
		}
		//printf("\n");
	}
}
...
void animatedDiskHandle(AnimatedDisk* animatedDisk, bool* isRunning,
						int* mIndex, int *move, int key, int *gameState)
{
	if (!animatedDisk->active) {
		...
		if (*mIndex == 2) {
			startDiskAnimation(move[0], move[1], gameState, animatedDisk);
			*mIndex = 0;
		}
	}
	else {
		updateAnimation(animatedDisk, gameState);
	}
}

void drawScreen(bool isEndgameScreen, bool* isRunning,
				AnimatedDisk* animatedDisk, int* gameState, int* move,
				int* mIndex)
{
	...
	if (!isEndgameScreen) {
		...
		animatedDiskHandle(animatedDisk, isRunning, mIndex, move, key, gameState);

		gfx_filledRect(0, 0, screenW - 1, screenH - 1, BLACK);
		gfx_filledRect(0, screenH - 10, screenW, screenH, YELLOW);
		drawPoles();
		drawDisks(gameState, animatedDisk);
...
int main(int argc, char* argv[])
{
	...
	bool isEndgameScreen = false;
	bool running = true;
	int gameState[DISKS_NO][POLES_NO];
	...
	while (running) {
		...
		drawScreen(isEndgameScreen, &running, &animatedDisk, *gameState, move,
				   &mIndex);
		isEndgameScreen = checkWon(*gameState);
		gfx_updateScreen();
